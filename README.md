...
## Fork: GBM-backed glamor + Intel Xe (hardware capture)

This fork of xorgxrdp adds the plumbing for **hardware video capture/encode** - the
companion to the VA-API H.264 encoder in
[pletch/xrdp-vaapi-encode](https://github.com/pletch/xrdp-vaapi-encode). The
`xrdp_accel_assist` hook itself is already upstream, and the screen still reaches the
encoder through it: the X pixmap handshake, bound as a texture in the helper's own GL
context. xorgxrdp passes no dma-buf to the helper; Mesa implements that binding with
DRI3, which shares the screen's buffer underneath. This fork makes glamor work on the
Intel `xe` kernel driver and adds capture pacing and AVC444 signalling on top.

### What changed
* **glamor: back the screen pixmap with GBM BOs**
...
* **glamor: stop using raw DRM ioctls** (FlyGoat)
  ...
  These three are FlyGoat's fixes for glamor and DRI3 on `xe`, open upstream as
...
VA-API H.264 with roughly 5x less server CPU than software x264. The encoder probes
...
guaranteed to be the one you want, or the same one across reboots. The helper binds the
screen pixmap through the X server, so its GL context and colour-conversion shaders run
on xorgxrdp's GPU. It opens VA-API separately, on its own device, and imports the
shaders' NV12 output there as a dma-buf. If the two are different GPUs, that import
fails and the session falls back to software encoding.
